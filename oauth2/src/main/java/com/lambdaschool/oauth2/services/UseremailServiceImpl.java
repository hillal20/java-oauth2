...
public class UseremailServiceImpl implements UseremailService {
    ...
    public Useremail findUseremailById(long id) {
        return useremailrepos.findById(id).orElseThrow(() -> new EntityNotFoundException("Useremail with id " + id + " Not Found!"));
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void delete(long id, boolean isAdmin) {
        if (useremailrepos.findById(id).isPresent()) {
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            if (useremailrepos.findById(id).get().getUser().getUsername().equalsIgnoreCase(authentication.getName()) || isAdmin) {
    ...
    public Useremail update(long useremailid, String emailaddress, boolean isAdmin) {
        ...
        if (useremailrepos.findById(useremailid).isPresent()) {
            if (useremailrepos.findById(useremailid).get().getUser().getUsername().equalsIgnoreCase(authentication.getName()) || isAdmin) {
