...
@Transactional // if we need data we use or it will die
@Component // these 2 lines makes implement the seed data
public class SeedData implements CommandLineRunner
{
    ...
    public void run(String[] args) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            users = new ArrayList<>();
            users.add(new UserRoles(new User(), r2));
            fakeUser = new User(nameFaker.name().username(), "password", nameFaker.internet().emailAddress(), users);
